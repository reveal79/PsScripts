# Get-DepartmentDevices.ps1
#
# A PowerShell script to inventory Windows devices used by employees reporting to a specific manager 
# by querying Entra ID (Azure AD) sign-in logs.
#
# Author: Don Cook
#
# OVERVIEW:
# This script creates a comprehensive device inventory by:
...
#
# KEY FEATURES:
# - Progressive search strategy: Starts with recent logs (24hrs) and expands only when needed
# - Robust error handling: Multiple fallback strategies for API timeouts
# - Smart "give up" logic: Checks account activity before exhaustive searching
# - Flexible reporting: Console output, CSV export, and optional HTML report
# - Manager inclusion: Option to include the manager's devices in the report
# - Email validation: Prevents typos in manager email addresses
#
# USAGE:
...
#   .\Get-DepartmentDevices.ps1 -ManagerUpn "manager@company.com" -ExportHtmlReport
#
# Custom lookback periods:
#   .\Get-DepartmentDevices.ps1 -ManagerUpn "manager@company.com" -DaysBack 5 -MaxDaysBack 10
#
# DEPENDENCIES:
...
# - Uses connection timeout control to prevent API hangups
# - Implements exponential backoff with retries
# - Targets recent data first for faster results
# - Runs preliminary checks to avoid unnecessary searches
#
# Extremely resilient against Microsoft Graph API timeouts, which are common when querying sign-in logs in hybrid environments.
...
param (
    ...
    [Parameter(Mandatory=$false)]
    [string]$OutputPath = ".\DeviceInventory_$(Get-Date -Format 'yyyyMMdd').csv",
...
function Initialize-RequiredModules {
    $requiredModules = @("Microsoft.Graph.Users", "Microsoft.Graph.Identity.SignIns", "Microsoft.Graph.Authentication")
...
function Get-DirectReports {
    ...
    try {
        ...
        Write-Host "Found $($users.Count) direct reports for manager: $($manager.DisplayName)" -ForegroundColor Green
        return $users
    }
    catch {
...
function Process-DeviceInformation {
    ...
    foreach ($device in $deviceSignIns) {
        ...
        Write-Host "    Device: $($deviceInfo.DeviceName) - $($deviceInfo.OperatingSystem)" -ForegroundColor White
    }
...
"@
    ...
    $htmlRows = $DeviceData | ForEach-Object {
        $rowClass = if ($_.DeviceName -like "No *") { ' class="warning"' } else { '' }
...
try {
    ...
    if ($managersDirectReports -and ($managersDirectReports.Count -gt 0)) {
        ...
        if (!$ExportCsvOnly) {
            ...
            Write-Host "`n=== DEVICE INVENTORY RESULTS ===" -ForegroundColor Cyan
            $userDevices | Format-Table -Property UserDisplayName, UserPrincipalName, DeviceName, OperatingSystem, LastSignIn -AutoSize
        }
        ...
        if ($ExportHtmlReport) {
            $htmlPath = New-HtmlReport -DeviceData $userDevices -ManagerUpn $ManagerUpn -TotalUsers $managersDirectReports.Count -UsersWithDevices $usersWithDevices -UsersNoDevices $usersNoDevices -TotalDevices $totalDevicesFound
